...
from sick_rl_log_analysis import do_analyse_sick_rl

# choose model
network_type = cfg.network_type
if network_type == 'resan_base':
    from src.model.model_resan_base import ModelResanBase as Model
elif network_type == 'resan':
    from src.model.model_resan import ModelResan as Model
...
def train():
    ...
    for sample_batch, batch_num, data_round, idx_b in train_data_obj.generate_batch_sample_iter(num_steps):
        ...
        loss, summary = model.step(
            sess, sample_batch, get_summary=if_get_summary, global_step_value=global_step)
        if global_step % 100 == 0:
            _logger.add('data round: %d: %d/%d, global step:%d -- loss: %.4f, loss_rl: %.4f' %
        ...
        if global_step > cfg.end_only_rl and global_step % (cfg.eval_period or steps_per_epoch) == 0:
            # ---- dev ----
            (dev_loss_sl, dev_loss_rl), dev_prec, (dev_pearson, dev_spearman, dev_mse) = evaluator.get_evaluation(
                sess, dev_data_obj, global_step
            )
            _logger.add('==> for dev, loss: %.4f %.4f, prec: %.4f, prsn: %.4f, sprmn: %.4f, mse: %.4f' %
                        (dev_loss_sl, dev_loss_rl, dev_prec, dev_pearson, dev_spearman, dev_mse))
            # ---- test ----
            (test_loss_sl, test_loss_rl), test_prec, (test_pearson, test_spearman, test_mse) = evaluator.get_evaluation(
                sess, test_data_obj, global_step
            )
            _logger.add('~~> for test, loss: %.4f %.4f, prec: %.4f, prsn: %.4f, sprmn: %.4f, mse: %.4f' %
                        (test_loss_sl, test_loss_rl, test_prec, test_pearson, test_spearman, test_mse))
    ...
    do_analyse_sick_rl(_logger.path)
