...
from src.nn_utils.nn import bn_dense_layer, dropout, linear, pooling_with_mask
from src.nn_utils.general import exp_mask_for_high_rank, mask_for_high_rank
from src.nn_utils.rl.nn import reduce_data_rep_max_len
    ...
    with tf.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        attn_result = tf.cond(
            ...
            lambda: self_attention_for_selected_head(
                rep_head_tensor, head_org_idx, sl_head, rep_head_mask,
                rep_dep_tensor, dep_org_idx, sl_dep, rep_dep_mask,
                keep_prob, is_train, direction, ivec, tensor_dict, name_prefix
            )
        )
        ...
        if keep_unselected:
            ...
            with tf.name_scope('fast_pooling_for_un_head'):
                pooling_result = pooling_with_mask(rep_map, rep_mask, 'mean')  # bs, hn
                pooling_result = tf.tile(tf.expand_dims(pooling_result, 1), [1, sl_unhead, 1])  # bs, sluh, hn
        ...
        if tensor_dict is not None:
            tensor_dict[name_prefix+'_fusion_gate'] = fusion_gate
...
def self_attention_for_selected_head(
        rep_head_tensor, head_org_idx, sl_head, rep_head_mask,
        rep_dep_tensor, dep_org_idx, sl_dep, rep_dep_mask,
        keep_prob, is_train, direction, ivec, tensor_dict=None, name_prefix=''
):
    # data for self-attention
    rep_dep_tensor_dp = dropout(rep_dep_tensor, keep_prob, is_train)
    rep_head_tensor_dp = dropout(rep_head_tensor, keep_prob, is_train)
    ...
    if tensor_dict is not None:
        tensor_dict[name_prefix + '_attn_score'] = attn_score

    return attn_result


def mean_pooling_for_unselected_fast(
